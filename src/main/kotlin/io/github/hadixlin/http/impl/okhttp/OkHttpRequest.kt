...
/** @author hadix
 */
class OkHttpRequest internal constructor(url: String, http: OkHttp) : AbstractHttpRequest(url, http) {
	...
	private fun httpUrl(): HttpUrl {
		...
		params.forEach { (name, values) -> values.forEach { v -> urlBuilder.addQueryParameter(name, v.toString()) } }
		return urlBuilder.build()
	}

	private val requestBody: RequestBody?
		get() = if (MultipartBody.FORM == mimeType()) {
	...
	override fun <T> submitForStream(
		...
		errorHandler: ResponseHandler<HttpException, T>): T {
		try {
			submit().use { body ->
	...
	private fun createMultiRequestBody(): RequestBody {
		...
		params.forEach { (name, values) ->
			values.forEach { value ->
				when (value) {
					is File -> builder.addFormDataPart(name, value.name, MultipartBody.create(null, value))
					is ByteArray -> builder.addFormDataPart(name, null, MultipartBody.create(null, value))
	...
	private fun createTextRequestBodyUseParams(): RequestBody {
		...
		params.forEach { (name, values) -> values.forEach { v -> bodyBuilder.add(name, v.toString()) } }
